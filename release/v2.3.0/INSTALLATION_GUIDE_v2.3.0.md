# Quick Installation Guide - v2.3.0

## 🚀 Fast Installation (Recommended)

### Using Hammerhead App (Easiest)

1. **Download** `Enhance-d_Club_Glucose_Karoo_v2.3.0.apk` from this release
...
1. **Download** `Enhance-d_Club_Glucose_Karoo_v2.3.0.apk`
2. **Enable USB debugging** on your Karoo:
   - Settings → About → Tap "Build Number" 7 times
   - Settings → Developer Options → Enable "USB Debugging"
3. **Connect** via USB to your computer
4. **Install**: `adb install Enhance-d_Club_Glucose_Karoo_v2.3.0.apk`
...
- **Fixed "No Sensor" Display Issue**: Glucose mg/dL and mmol/L widgets now correctly display values when data is fresh
- **Improved State Management**: Widgets correctly show "no sensor" only when data is actually stale (>10 minutes old)
- **Better Reliability**: All glucose data fields now work consistently

### Before v2.3.0:
- Widgets showed "no sensor" even when app displayed correct values
- Inconsistent behavior between app and widgets

### After v2.3.0:
- Widgets correctly display glucose values when data is fresh
- Widgets correctly show "no sensor" when data is stale
- Consistent behavior across all data fields
